...
Paper 3 presents a number of statistical methods including Bland-Altman analysis for method comparison, bootstrapping, a mixed-effects model and Bayesian statistics. In this dissertation, I will only elaborate on Bayesian statistics and explain the Bayesian mixed-effects model presented in the paper.
...
A main aim in Paper 3 was to understand how tidal volume ($V_T$) and respiratory rate (RR) impact PPV. The experiment had a hierarchical structure, where PPV was measured repeatedly in each subject, but with different RR and $V_T$ settings; we also calculated PPV with two different methods (PPV~Classic~ and PPV~GAM~) and wanted to know if $V_T$ and RR impact PPV differently depending on the method. In Paper 3, we used a Bayesian generalized mixed-effects model to find the effects of $V_T$, RR and PPV-method on PPV. Below, I will walk through this model.
...
We specified a linear model for the expected PPV value, $\mu$:
$$
log(\mu_i) = 
...
\alpha_{subject[i]}.
$$
Specifically, the linear model is for $log(\mu)$. Therefore, the exponential of the parameters correspond to relative effects on PPV. $exp(\beta1)$ and $exp(\beta2)$ are the effects of each $V_T$- and RR setting for each PPV method relative to the expected PPV at $V_T$=10 ml kg^-1^ and RR=10 min^-1^. $exp(\beta0)$ is the median PPV at $V_T$=10 ml kg^-1^ and RR=10 min^-1^ for each method. $\alpha$ is a random effect of each subject. This random effect captures that subjects have individual levels of PPV across ventilator settings---it captures the between-individual variation in PPV.

The random effect, $\alpha$, was estimated for each subject. We assumed that all $\alpha$ for all subjects is draw from a shared normal distribution:
$$
