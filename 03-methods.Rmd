...
For example, we may believe that tidal volume ($V_T$) affects PPV through a number of intermediate steps. But often, we are not interested in the intermediate effects—they may not even be identifiable from the data we have available. Instead we may assume that if we vary $V_T$ in a single individual, there is a linear relationship between $V_T$ and PPV:
$$
PPV = \alpha + \beta V_T.
$$
If we measure PPV thrice, and only change $V_T$ between measurements, we expect that PPV will vary according to the linear model above. Additionally, we also anticipate some variation in PPV that is not explained by the change in $V_T$. This additional variation can be due to a physiological change over time, unrelated to the $V_T$ change, or due to imprecise measurements. Unexplained variation is traditionally represented by the greek letter $\epsilon$, creating the statistical linear model:
$$
...
Paper 3 investigates PPV’s ability to predict the CO response to a fluid bolus, and especially, how ventilator settings affect this ability. We expected that PPV at $V_T$=10 ml kg^-1^ and RR=10 min^-1^ would predict fluid responsiveness with high accuracy, but that the accuracy would fall with lower $V_T$ and higher RR. The CO response was evaluated from pulse contour analysis; I will, therefore, present a section about the accuracy of pulse contour derived CO.
...
A main aim in Paper 3 is to understand how tidal volume ($V_T$) and respiratory rate (RR) impact PPV. The experiment has a hierarchical structure, where PPV is measured repeatedly in each subject, but with different RR and $V_T$ settings; we also calculate PPV with two different methods (PPV~Classic~ and PPV~GAM~) and want to know if $V_T$ and RR impact PPV differently depending on the method. In Paper 3, we use a Bayesian generalized mixed-effects model to find the effects of $V_T$, RR and PPV-method on PPV. Below, I will walk through this model.
...
Specifically, the linear model is for $log(\mu)$. Therefore, the exponential of the parameters correspond to relative effects on PPV. $e^{\beta1}$ and $e^{\beta2}$ are the effects of each $V_T$ and RR setting for each method relative to the effect at $V_T$=10 ml kg^-1^ and RR=10 min^-1^.  $e^{\beta0}$ is the median PPV at $V_T$=10 ml kg^-1^ and RR=10 min^-1^ for each method. $\alpha$ is a random effect of each subject. This random effect captures that subjects have individual levels of PPV across ventilator settings---it captures the between-individual variation in PPV.
