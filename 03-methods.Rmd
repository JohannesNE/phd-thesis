...
A main aim in Paper 3 was to understand how $V_T$ and RR impact PPV. The experiment had a hierarchical structure, where PPV was measured repeatedly in each subject, but with different RR and $V_T$ settings; we also calculated PPV with two different methods (PPV~Classic~ and PPV~GAM~) and wanted to know if $V_T$ and RR impact PPV differently depending on the method. In Paper 3, we used a Bayesian generalized mixed-effects model to find the effects of $V_T$, RR and PPV-method on PPV. Below, I will walk through this model.
